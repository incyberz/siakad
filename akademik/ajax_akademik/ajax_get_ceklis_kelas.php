...
$s = "SELECT a.id as id_kelas_ta FROM tb_kelas_ta a 
...
while ($d=mysqli_fetch_assoc($q)) {
  $assigned_kelas_angkatan[$i] = $d['id_kelas_ta'];
  $i++;
}
...
# GET KELAS IN THIS JADWAL
# ===================================================
$s = "SELECT a.id as id_kelas_ta,b.kelas 
FROM tb_kelas_ta a 
...
$ceks = '';
while ($d=mysqli_fetch_assoc($q)) {
  if(in_array($d['id_kelas_ta'],$assigned_kelas_angkatan)) continue;
  $ceks .= "<div><label><input type=checkbox id='$d[id_kelas_ta]' name='$d[id_kelas_ta]'> $d[kelas]</label></div>";
}

...
<div class="blok_filter mb2">
  ...
  <div class=hideit>Kalender <span class=debug id=last_keyword>last_keyword</span></div>
  <div class=hideit>
    <input class="form-control" id=keyword placeholder="Kurikulum ...">
  </div>

</div>
<div id="blok_ajax_hasil"></div>
...
<script>
  $(function(){
    $("#keyword").keyup(function(){
      ...
      let link_ajax = `ajax_akademik/ajax_get_list_kurikulum_semester.php?keyword=${keyword}&id_prodi=${id_prodi}&angkatan=${angkatan}&`;
      $("#debug1").text(link_ajax);
      $.ajax({
        url:link_ajax,
        success:function(a){
          $("#blok_ajax_hasil").html(a);
        }
      })
